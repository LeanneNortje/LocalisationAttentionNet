    ...
    if rank == 0: 
        ...
        with torch.no_grad():
            ...
            for entry in tqdm(samples):
                ...
                wav_fn = Path('/storage/Datasets/flickr_audio/wavs') / Path(str(Path(entry['wave']).stem) + '.wav') 
                ...
                sim = torch.rand(image_output.size(0))

                similarities = torch.zeros(len(iVOCAB), device=image_output.device)
                ...
                scores = torch.rand((image_output.size(0), 1, 512))
